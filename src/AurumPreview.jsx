...
function generateCodeFromPrompt(p) {
  ...
  if (/particles?|stars?|galaxy|spiral/.test(prompt)) {
    return `${base}const P=400;const ps=[];for(let i=0;i<P;i++){ps.push({a:Math.random()*Math.PI*2,r:5+Math.random()*150,s:0.002+Math.random()*0.01});}function draw(){ctx.fillStyle='#000';ctx.fillRect(0,0,W,H);ctx.translate(W/2,H/2);for(const p of ps){p.a+=p.s;const x=Math.cos(p.a)*p.r,y=Math.sin(p.a)*p.r;ctx.fillStyle='rgba(241,197,80,0.9)';ctx.fillRect(x,y,2,2);}ctx.setTransform(1,0,0,1,0,0);requestAnimationFrame(draw);}draw();`;
  }
  ...
  if (/bounc|ball|orbs?/.test(prompt)) {
    return `${base}const N=12,balls=[];for(let i=0;i<N;i++){balls.push({x:Math.random()*W,y:Math.random()*H,vx:(Math.random()*2-1)*3,vy:(Math.random()*2-1)*3,r:6+Math.random()*8})}function draw(){ctx.fillStyle='#000';ctx.fillRect(0,0,W,H);for(const b of balls){b.x+=b.vx;b.y+=b.vy;if(b.x<b.r||b.x>W-b.r) b.vx*=-1;if(b.y<b.r||b.y>H-b.r) b.vy*=-1;ctx.beginPath();ctx.arc(b.x,b.y,b.r,0,Math.PI*2);ctx.fillStyle='#f1c550';ctx.shadowColor='#f1c55088';ctx.shadowBlur=10;ctx.fill();ctx.shadowBlur=0;}requestAnimationFrame(draw);}draw();`;
  }
